...
const MISSIONS = [
  {
    id: 1, name: 'Defeat the Leader', icon: '⚔',
    goal: 'Knock out the Quest Leader.',
    setup: 'The Quest Leader is placed in the Boss Room. Heroes enter from the Start Tile.',
    rules: [
      'The Quest Leader activates every turn once revealed.',
      'Heroes win when the Quest Leader is knocked out.',
      'Dark Player wins if all heroes fall.',
    ],
    settings: ['dungeon','civilised','outdoor'], allowRandom: true,
  },
  {
    id: 2, name: 'Steal the Relic', icon: '💎',
    goal: 'Steal the relic from the Leader and escape.',
    setup: 'The Quest Leader holds the relic and is placed in the Boss Room.',
    rules: [
      'To steal: adjacent hero rolls Skilled Hands vs Perception. On success, hero takes relic.',
      'The hero carrying the relic must exit via the Start Tile.',
      'Dark Player wins if all heroes fall or turn limit (45) expires.',
    ],
    settings: ['dungeon','civilised'], allowRandom: true, turnLimit: 45,
  },
  {
    id: 3, name: 'Rescue the Prisoner', icon: '⛓',
    goal: 'Free the prisoner and escort them to the exit.',
    setup: 'Place the prisoner token in a locked Cell. Keys are distributed randomly among monster leaders.',
    rules: [
      'A hero adjacent to the prisoner may spend 1 action to escort them.',
      'The prisoner moves with the escort hero.',
      'If the escort hero is KO\'d, another hero may take over next activation.',
      'Heroes win when prisoner exits from the Start Tile. Dark wins if prisoner is killed or turn limit expires.',
    ],
    settings: ['dungeon','civilised'], allowRandom: true, turnLimit: 40,
  },
  {
    id: 4, name: 'Destroy the Artefact', icon: '💥',
    goal: 'Reach the Artefact and destroy it before it empowers the enemy.',
    setup: 'Place the Artefact token on a special altar in the deepest room. Monsters gain +1 Attack each time a hero is KO\'d until the artefact is destroyed.',
    rules: [
      'Destroying the artefact requires 2 actions and a Strength roll (7+).',
      'Each failed destruction roll spawns one additional monster.',
      'Heroes win when artefact is destroyed. Dark wins if all heroes are KO\'d.',
    ],
    settings: ['dungeon'], allowRandom: true,
  },
  {
    id: 5, name: 'Escort the Messenger', icon: '📜',
    goal: 'Keep the messenger alive until they reach the exit.',
    setup: 'The messenger starts at the Start Tile. Place 3 Ambush tokens face-down in corridors — when revealed, each spawns 1d3 monsters.',
    rules: [
      'The messenger is a neutral figure with 2 Wounds and no attacks.',
      'A hero must remain within 2 squares of the messenger each turn.',
      'If the messenger is killed, the mission is lost.',
      'Heroes win when the messenger exits via the designated exit tile.',
    ],
    settings: ['dungeon','civilised','outdoor'], allowRandom: true, turnLimit: 35,
  },
  {
    id: 6, name: 'Survive the Night', icon: '🌑',
    goal: 'Survive until dawn — hold out for the required number of turns.',
    setup: 'Heroes begin in a central room. All exits are sealed. Monsters spawn each turn from the darkness.',
    rules: [
      'At the start of each Dark Player turn, spawn monsters equal to the turn number ÷ 3 (round up), minimum 1.',
      'Doors may be barricaded (1 action, 3 Wounds). Barricades slow monster movement.',
      'Heroes win by surviving to Turn 20. Dark wins if all heroes are KO\'d.',
    ],
    settings: ['dungeon','civilised'], allowRandom: false, turnLimit: 20,
  },
  {
    id: 7, name: 'Plunder the Vault', icon: '🏆',
    goal: 'Collect as much treasure as possible and escape.',
    setup: 'Scatter 6 chest tokens randomly across revealed rooms. Each chest contains 1d6+2 gold when opened (1 action, adjacent).',
    rules: [
      'Heroes may carry unlimited gold but only one chest item each.',
      'Monsters are attracted to opened chests and move toward them.',
      'Heroes win if at least 3 chests are looted and all loot-carriers exit.',
      'Dark wins if fewer than 3 chests are looted before heroes retreat.',
    ],
    settings: ['dungeon','civilised'], allowRandom: true, turnLimit: 35,
  },
  {
    id: 8, name: 'Assassinate the Champion', icon: '🗡',
    goal: 'Eliminate the Champion without alerting the full garrison.',
    setup: 'The Champion patrols a fixed route. Other monsters are dormant until alerted.',
    rules: [
      'Monsters become alerted if: a hero is seen, a fight begins in an open room, or a door is broken.',
      'Once alerted, all dormant monsters activate immediately.',
      'Heroes win if Champion is KO\'d and at least 2 heroes escape. Dark wins if all heroes fall.',
    ],
    settings: ['dungeon','civilised'], allowRandom: true,
  },
  {
    id: 9, name: 'Retrieve the Tome', icon: '📖',
    goal: 'Find and retrieve the ancient tome from its resting place.',
    setup: 'The tome is hidden in one of three possible rooms — only revealed when that room is explored.',
    rules: [
      'When the third possible room is revealed, the tome is automatically placed there.',
      'A hero carrying the tome moves at -1 Speed.',
      'If the carrier is KO\'d, the tome remains where they fell.',
      'Heroes win when the tome exits via the Start Tile.',
    ],
    settings: ['dungeon','civilised'], allowRandom: true, turnLimit: 50,
  },
  {
    id: 10, name: 'The Gauntlet', icon: '🏃',
    goal: 'All heroes must reach the Exit before the turn limit.',
    setup: 'Place the Exit token at the far end of the dungeon. Monsters spawn behind the heroes each turn.',
    rules: [
      'At the start of each Dark Player turn, spawn 1d3 monsters at the Start Tile.',
      'These monsters always move toward the nearest hero.',
      'Heroes cannot double back past a sealed door.',
      'Heroes win when all surviving heroes exit. Dark wins if fewer than half exit.',
    ],
    settings: ['dungeon'], allowRandom: false, turnLimit: 25,
  },
  {
    id: 11, name: 'The Siege', icon: '🏰',
    goal: 'Defend the stronghold entrance for the required turns.',
    setup: 'Heroes are placed in a fortified room. Monsters attempt to break through barricaded doors (5 Wound points each).',
    rules: [
      'Doors may be repaired (1 action, Craftsmanship roll 4+).',
      'If both doors fall, 3 additional monsters spawn per turn until the dungeon is cleared.',
      'Heroes win if both doors survive to Turn 15, or all monsters are eliminated.',
      'Dark wins if all heroes fall.',
    ],
    settings: ['dungeon','civilised'], allowRandom: false, turnLimit: 15,
  },
  {
    id: 12, name: 'The Final Reckoning', icon: '💀',
    goal: 'Defeat the Boss and all Champions before the dungeon collapses.',
    setup: 'Place the Boss in the Boss Room with 2 Champions.',
    rules: [
      'Champions each have a unique special ability.',
      'The Boss is immune to Fear effects.',
      'Heroes win if Boss and both Champions are defeated before Turn 20.',
      'Dark wins if all heroes are KO\'d or the turn limit expires.',
    ],
    settings: ['dungeon'], allowRandom: false, turnLimit: 20,
  },
];
...
/* ═══════════════════════════════════════
   ...
   Called by modules/custom.html after mission selection
   ═══════════════════════════════════════ */
window.loadBuilderDungeon = function(builderPlaced, builderConns, mission, settingCat, sessionName) {
  if(!builderPlaced||!builderPlaced.length) return;

  // Resolve setting (handle 'random')
  var settings = mission&&mission.settings||['dungeon','civilised','outdoor'];
  var resolvedSetting = settingCat==='random'
    ? settings[Math.floor(Math.random()*settings.length)]
    : (settingCat||'dungeon');

  // Apply setting to generator
  var catSel=document.getElementById('sel-category');
  if(catSel){catSel.value=resolvedSetting; activeCategory=resolvedSetting;}
  activeTheme='any';
  rebuildDecks();

  // Reset generator state
  levelNum++;
  seed=Math.floor(Math.random()*99999)+1000; rng=mkRng(seed);
  placed=[]; connections=[]; logLines=[]; selectedTile=null; canalExits=[]; placedElements=[];
  occ=new Set();

  // Load tiles
  builderPlaced.forEach(function(bp){
    var pl={id:bp.id,tile:bp.tile,gx:bp.gx,gy:bp.gy,w:bp.w,h:bp.h,rot:bp.rot||0,
      revealed:bp.revealed||false,entities:[],isBoss:bp.isBoss||false,isObjective:bp.isObjective||false};
    placed.push(pl); occAdd(pl);
  });

  // Load connections
  connections=builderConns.map(function(c){
    return{fromId:c.fromId,toId:c.toId,side:c.side,slot:c.slot,doorType:c.doorType||'single-door'};
  });

  // Set active mission
  if(mission){
    activeMission={
      id:mission.id, name:sessionName||mission.name,
      goal:mission.goal||'Explore your dungeon.',
      icon:mission.icon||'★',
      turnsLeft:mission.turnLimit||null,
      chosenSetting:resolvedSetting,
  ...
  if(headerP) headerP.textContent=(sessionName||'Custom Dungeon').toUpperCase()+' · '+resolvedSetting.toUpperCase();
  ...
  if(mName) mName.textContent=(mission&&mission.icon||'★')+' '+(sessionName||'Custom Dungeon');
  var mGoal=document.getElementById('s-mission-goal');
  if(mGoal) mGoal.textContent=(mission&&mission.goal)||'Explore your dungeon.';
  ...
  if(mission&&mission.turnLimit){
    if(turnRow) turnRow.style.display='flex';
    if(turnEl){turnEl.textContent=mission.turnLimit;turnEl.style.color='#e8b040';}
